...
**Target device:** Galaxy S22 Ultra (SM-S908B), One UI 7 / Android 14, Exynos
2200, rooted with Magisk, LSPosed installed.
...
| Target | Why |
| --- | --- |
| `VotesStorage.updateVote` / `updateGlobalVote` | In Android 14 every constraint on the allowed refresh-rate range is a `Vote` filed under a priority, and the result is the intersection. Whatever drops the panel to 60 Hz under DeX has to appear here. |
| `DisplayModeDirector.getDesiredDisplayModeSpecs`, `selectBaseMode` | What the framework concluded from those votes. |
...
| `DisplayManagerService` mode/refresh methods | Including `requestDisplayStateInternal`, Android 14's name for what older notes call `requestDisplayPower`. |
...
Needs JDK 17 and an Android SDK with API 34. No network dependency beyond the
Gradle/AGP download; the Xposed API is vendored as compile-only stubs in
`xposed-stubs/`, so there is nothing to fetch from `api.xposed.info`.
...
Android Studio works too — open the project root.

> No APK is committed to this repo. It was authored in an environment without
> the Android SDK, so it has been typechecked against a real Android 14
> framework jar and its discovery logic dry-run against actual AOSP 14 bytecode
> (see `docs/PROBE_PLAN.md`), but never compiled to a DEX or run on a device.
> Treat the first boot with the module enabled accordingly.
...
(Snapdragon), this targets an S22 Ultra on One UI 7 / Android 14 (Exynos). Their
